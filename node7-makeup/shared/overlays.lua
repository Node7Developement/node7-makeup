-- Native head-overlay catalogs used exclusively by node7-makeup.
...
-- The source catalog mixes male and female eyebrow assets. Keep only the
-- native assets for the active multiplayer ped sex.
Node7MakeupOverlayStyleMap = {
    eyebrows = {
        male = { 1, 2, 3, 4, 5, 7, 8, 9, 14, 17, 18, 20, 21, 22, 23, 24 },
        female = { 6, 10, 11, 12, 13, 15, 16, 19 }
    }
}

function Node7GetOverlayList(key, gender)
    local source = type(overlays_info) == 'table' and overlays_info[key] or nil
    if type(source) ~= 'table' then return {} end
    if key ~= 'eyebrows' then return source end

    local indexes = Node7MakeupOverlayStyleMap.eyebrows[gender == 'male' and 'male' or 'female'] or {}
    local filtered = {}
    for _, index in ipairs(indexes) do
        if source[index] then filtered[#filtered + 1] = source[index] end
    end
    return filtered
end
